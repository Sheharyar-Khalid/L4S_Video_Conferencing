...
# on both the host and the network namespace 'router_ns'
...
print_sysctl() {
    local scope="$1"  # "Host" or "router_ns"
    ...
    for var in "${variables[@]}"; do
        if [[ "$scope" == "Host" ]]; then
            ...
            if ip netns exec router_ns sysctl -n "$var" &>/dev/null; then
                value=$(ip netns exec router_ns sysctl -n "$var")
            else
...
print_tc_qdisc() {
    ...
    if [[ "$scope" == "Host" ]]; then
        ...
        interfaces=$(ip netns exec router_ns ip -o link show | awk -F': ' '{print $2}' | grep -v "lo")
        if [[ -z "$interfaces" ]]; then
            log "No relevant interfaces found in router_ns."
            return
        fi
        for iface in $interfaces; do
            base_iface="${iface%%@*}"
            echo "Interface: $base_iface"
            ip netns exec router_ns tc qdisc show dev "$base_iface"
            echo ""
        done
    fi
    echo ""
...
# Check if 'router_ns' namespace exists
if ip netns list | grep -qw "router_ns"; then
    # Print sysctl variables for router_ns
    print_sysctl "router_ns"

    # Print tc qdisc for router_ns
    print_tc_qdisc "router_ns"
else
    log "Network namespace 'router_ns' does not exist. Skipping verification for router_ns."
fi
